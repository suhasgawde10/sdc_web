    ...
    if ($radio_value != "Free Trail (5 days)") {
        ...
        if ($countryName == 'India' OR $countryName == '') {
            $payment_data .= '<tr class="validate_referral_code"><td><a data-toggle="modal"
                                   data-target="#user_referral_code">Have a Referral code?</a></td><td class="text-center" colspan="2"><a data-toggle="modal"
...
<input type="hidden" name="razorpay_payment_id" id="razorpay_payment_id">
                                        ...
                                        <button class="btn btn-primary form-control submit_btn" id="rzp-button1"
                                                name="upgrade_plan" type="button">Pay now</button>
                                        </td>
                                            </tr>';
...
if (isset($_POST['refereal_code'])) {
    ...
    if ($validateReferealCode) {
        echo '
        <td>' . $refereal_code . '</td>
                                   <td class="text-right"><a onclick="InvalidReferralCode()" href="javascript:void(0);"><i class="fa fa-trash" aria-hidden="true"></i></a></td>
        ';
...
if (isset($_POST['coupon_code'])) {
    ...
    if ($validateCouponCode) {
        echo '<td>' . $coupon_code . '</td>
                                   <td class="text-right"><a onclick="InvalidDealerCode()" href="javascript:void(0);"><i class="fa fa-trash" aria-hidden="true"></i></a></td>
                                   </tr>';
    }else {
