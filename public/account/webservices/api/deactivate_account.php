...
if (isset($_POST)) {
    ...
    if (isset($_POST["user_id"]) && isset($_POST["reason_leaving"]) && isset($_POST["further"]) && isset($_POST["password"])) {
        if ($_POST["user_id"] != '' && $_POST["reason_leaving"] != '' && $_POST["further"] != '' && $_POST["password"] != '') {
            ...
            if ($error == false) {
                try {
                    $insert = $manage->deactivateUserAccount($reason_leaving, $further, "deactivated", 2, $id);
                    if ($insert) {
                        $response["status"] = true;
                        $response["code"] = 185;
                        $response["message"] = "You have deactivated successfully";
                    } else {
                        $response["code"] = 191;
                        $response["message"] = "Issue while deactivated.";
                    }
                } catch (Exception $e) {
                    $response["message"] = "Something went wrong.";
                }
