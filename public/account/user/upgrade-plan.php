...
if (isset($_POST['upgrade_plan'])) {
    ...
    if (isset($_POST['dealer_code']) && $_POST['dealer_code'] != "") {
        ...
        $dealer_payment = $total_amount/2;
        $get_user_data = $manage->displayAllUserByID($security->decrypt($_GET['user_id']));
        $dealer_profile = $manage->getDealerProfile($dealer_code);
        ...
        $insertUserSubscription = $manage->insertMannualSubscriptionData($security->decrypt($_GET['user_id']), $_SESSION['new_year'],$taxable_amount, $taxable_amount, $date, $expiry_date, $status_success, $dealer_code, $refrenced_by, $active_plan, $invoice_number, $dealer_discount, $total_tax, $total_amount, $paymentBrand, $paymentMode, $custBankId, $timestamp,$payment_type,$invoice_name,$email,$user_gstno,$user_pan_no,FROM_BILL,FROM_GSTNO,FROM_PAN,SAC_CODE);
        ...
        $update_dealer_payment = $manage->updateDealerPayment($dealer_payment, $dealer_code);
        $update_user_ref = $manage->updateUserReferenceById($dealer_code, $security->decrypt($_GET['user_id']));
        if ($update_user_ref) {
            $dealerError = true;
        }
    } elseif (isset($_POST['admin_discount']) && $_POST['admin_discount'] != "") {
        if ($_SESSION['new_year'] == "1 year") {
            $month = 12;
        } else if ($_SESSION['new_year'] == "3 year") {
            $month = 36;
        ...
        $active_plan = 1;
        $update_user_plan = $manage->updateUserPlanStatusById($security->decrypt($_GET['user_id']));
        $insertUserSubscription = $manage->insertMannualSubscriptionData($security->decrypt($_GET['user_id']), $_SESSION['new_year'],$taxable_amount, $taxable_amount, $date, $expiry_date, $status_success, $referral_code, $refrenced_by, $active_plan, $invoice_number, $admin_discount, $total_tax, $total_amount, $paymentBrand, $paymentMode, $custBankId, $timestamp,$payment_type,$invoice_name,$email,$user_gstno,$user_pan_no,FROM_BILL,FROM_GSTNO,FROM_PAN,SAC_CODE);
        $updateUserSubscription = $manage->updateUserExpiryDateById($expiry_date, $security->decrypt($_GET['user_id']));
        if ($updateUserSubscription) {
            $admin_discount = true;
        }
    } elseif (isset($_POST['referral_code']) && $_POST['referral_code'] != "") {
        if ($_SESSION['new_year'] == "1 year") {
            $month = 14;
        } else if ($_SESSION['new_year'] == "3 year") {
            $month = 38;
        ...
        $updateUserSubscription = $manage->updateUserExpiryDateById($expiry_date, $security->decrypt($_GET['user_id']));
        $get_referral_data = $manage->displayRelatedReferralUser($referral_code);
        if ($get_referral_data != null) {
            $referral_user_expiry_date = $get_referral_data['expiry_date'];
        }
        if ($referral_user_expiry_date != null && $referral_user_expiry_date >= $date) {
            $final_date = strtotime(date("Y-m-d", strtotime($referral_user_expiry_date)) . " +2 month");
            $final_date = date("Y-m-d", $final_date);
        } elseif ($referral_user_expiry_date != null && $referral_user_expiry_date <= $date) {
        ...
        $update_referral_user = $manage->update_referral_user($final_date, $referral_code);
        if ($update_referral_user) {
            $admin_discount = true;
        }
    } else {
        if ($_SESSION['new_year'] == "1 year") {
...
<td colspan="2" style="text-align: right">
          <strong> Bill for : </strong>' . $name . '<br><strong> Email : </strong>' . $email . '</td></td>
</tr>
<tr >
...
<script>
    function get_value(val) {
        ...
        var dataString = "radio_value=" + get_radio_value;
        $.ajax({
            type: "POST",
