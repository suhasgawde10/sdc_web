...
class ManageDealer
{
    ...
    function sendMailForDealer($toName, $toEmail, $subject, $message)
    {
        $sendMail = new sendMailSystem();
        $status = false;
        $sendMailStatus = $sendMail->sendMailForDealer($toName, $toEmail, "Digital Card", "noreply.digitalcard@gmail.com", $subject, $message);
        if ($sendMailStatus) {
            $status = true;
        } else {
            $status = false;
