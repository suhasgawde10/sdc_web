...
class ManageDesktopCard
{
    ...
    {
        $controller = new Controller();
        $query = "SELECT tbs.service_name,tbs.request_status,tbs.description,tbs.img_name,lt.email FROM tb_services as tbs INNER JOIN tb_user_profile as tbf ON tbs.user_id=tbf.id INNER JOIN tb_login as lt on lt.user_id=tbf.id WHERE tbf.custom_url='" . $custom_url . "' and tbs.status=1 order by tbs.position_order";
        /*echo $query;
        die();*/
    ...
    {
        $controller = new Controller();
        $query = "SELECT tb_image_slider.description,tb_image_slider.img_name FROM tb_image_slider INNER JOIN tb_user_profile ON tb_user_profile.id=tb_image_slider.user_id where tb_user_profile.custom_url='" . $custom_url . "' and tb_image_slider.status=1";
        /* echo $query;
         die();*/
        $result = $controller->genericSelectToIterate($query);
        return $result;
    ...
    {
        $controller = new Controller();
        $query = "SELECT tb_about_us.description,tb_about_us.img_name FROM tb_about_us INNER JOIN tb_user_profile ON tb_user_profile.id=tb_about_us.user_id where tb_user_profile.custom_url='" . $custom_url . "' and tb_about_us.status=1";
        /* echo $query;
         die();*/
        $result = $controller->genericSelectAlreadyIterated($query);
        return $result;
    ...
    function mdm_updateUserTextColor($user_id, $color)
    {
        $controller = new Controller();
        // $query = "select * from " . $this->logTable . " where user_id =" . $user_id . " and page_type = '" . $page_type . "' and date=curdate()";
        $query = "call mdm_updateUserTextColor(?,?)";
        $type = "is";
        $params = array($user_id, $color);
        $result = $controller->genericInsertUpdateDeleteUsingProcedure($query, $type, $params);
        return $result;
    }
    function mdm_updateBackgroundColor($user_id, $color)
    {
        $controller = new Controller();
        // $query = "select * from " . $this->logTable . " where user_id =" . $user_id . " and page_type = '" . $page_type . "' and date=curdate()";
        $query = "call mdm_updateBackgroundColor(?,?)";
        $type = "is";
        $params = array($user_id, $color);
        $result = $controller->genericInsertUpdateDeleteUsingProcedure($query, $type, $params);
        return $result;
    ...
    function insertUserCount($user_id, $page_type, $count, $ip_aadr, $city, $region, $countryName)
    {
        ...
        //$query = "call mdm_insertUserCount('$user_id', '$page_type', '$count','$ip_aadr','$city','$region','$countryName')";
        $type = "isissss";
    ...
    function updateUserCount($count, $user_id, $page_type, $ip_aadr, $city, $region, $countryName)
    {
        ...
        $query = "call mdm_insertUserCount(?,?,?,?,?,?,?)";
        // $query = "call mdm_updateUserCount('$count', '$page_type','$ip_aadr','$city','$region','$countryName','$user_id')";
        $type = "isssssi";
    ...
    function mdm_updateBackgroundTheme($theme, $custom_url)
    {
        $controller = new Controller();
        // $query = "update " . $this->profileTable. " set user_theme='" . $theme. "' where custom_url = '" . $custom_url . "'";
        $query = "call mdm_updateBackgroundTheme(?,?)";
        $type = "ss";
        $params = array($theme, $custom_url);
        $result = $controller->genericInsertUpdateDeleteUsingProcedure($query, $type, $params);
        return $result;
    ...
    function addClientsReviewWithRating($name, $description, $imgFile, $id, $rating_num, $time)
    {
        $controller = new Controller();
        ...
        $query = "call mu_addClientsReviewWithRating(?,?,?,?,?,?)";
        $type = "isssis";
        $params = array($id, $name, $description, $imgFile, $rating_num, $time);
        $status = $controller->genericInsertUpdateDeleteUsingProcedure($query, $type, $params);
        return $status;
    }
    ...
    function getSectionStatusByUser($user_id)
    {
        $controller = new Controller();
        //$results = false;
        // $sql_query = "select (SELECT IF(COUNT(id) > 0, 1, 0) FROM `tb_section_status` WHERE user_id =".$user_id." and section_id =10 and website IN(1) AND digital_card IN(1)) as profile_status,
        // (SELECT IF(COUNT(id) > 0, 1, 0) FROM `tb_section_status` WHERE user_id =".$user_id." and section_id =1 and website IN(1) AND digital_card IN(1)) as service_status, 
        // (SELECT IF(COUNT(id) > 0, 1, 0) FROM `tb_section_status` WHERE user_id =".$user_id." and section_id =11 and website IN(1) AND digital_card IN(1)) as product_status,
        // (SELECT IF(COUNT(id) > 0, 1, 0) FROM `tb_section_status` WHERE user_id =".$user_id." and section_id =2 and website IN(1) AND digital_card IN(1)) as gallery_status,
        // (SELECT IF(COUNT(id) > 0, 1, 0) FROM `tb_section_status` WHERE user_id =".$user_id." and section_id =4 and website IN(1) AND digital_card IN(1)) as client_status,
        // (SELECT IF(COUNT(id) > 0, 1, 0) FROM `tb_section_status` WHERE user_id =".$user_id." and section_id =6 and website IN(1) AND digital_card IN(1)) as team_status,
        // (SELECT IF(COUNT(id) > 0, 1, 0) FROM `tb_section_status` WHERE user_id =".$user_id." and section_id =7 and website IN(1) AND digital_card IN(1)) as bank_status 
        // from dual";

        $sql_query = "SELECT 
    SUM(CASE WHEN section_id = 10 THEN 1 ELSE 0 END) AS profile_status,
    SUM(CASE WHEN section_id = 1 THEN 1 ELSE 0 END) AS service_status,
    SUM(CASE WHEN section_id = 11 THEN 1 ELSE 0 END) AS product_status,
    SUM(CASE WHEN section_id = 2 THEN 1 ELSE 0 END) AS gallery_status,
    SUM(CASE WHEN section_id = 4 THEN 1 ELSE 0 END) AS client_status,
    SUM(CASE WHEN section_id = 6 THEN 1 ELSE 0 END) AS team_status,
    SUM(CASE WHEN section_id = 7 THEN 1 ELSE 0 END) AS bank_status FROM `tb_section_status` WHERE user_id = " . $user_id . " AND website IN (1) AND digital_card IN (1)";
    ...
    function getSectionImageDataByUser($user_id)
    {
        $controller = new Controller();
        // $sql_query = "select (SELECT section_img FROM `tb_section_icon` WHERE user_id =" . $user_id . " and section_id =10) as profile_status,
        // (SELECT section_img FROM `tb_section_icon` WHERE user_id =" . $user_id . " and section_id =1 ) as service_status, 
        // (SELECT section_img FROM `tb_section_icon` WHERE user_id =" . $user_id . " and section_id =11 ) as product_status,
        // (SELECT section_img FROM `tb_section_icon` WHERE user_id =" . $user_id . " and section_id =2 ) as gallery_status,
        // (SELECT section_img FROM `tb_section_icon` WHERE user_id =" . $user_id . " and section_id =4 ) as client_status,
        // (SELECT section_img FROM `tb_section_icon` WHERE user_id =" . $user_id . " and section_id =6 ) as team_status,
        // (SELECT section_img FROM `tb_section_icon` WHERE user_id =" . $user_id . " and section_id =7 ) as bank_status 
        // from dual";
        $sql_query = "SELECT 
    MAX(CASE WHEN section_id = 10 THEN section_img END) AS profile_status,
    MAX(CASE WHEN section_id = 1 THEN section_img END) AS service_status,
    MAX(CASE WHEN section_id = 11 THEN section_img END) AS product_status,
    MAX(CASE WHEN section_id = 2 THEN section_img END) AS gallery_status,
    MAX(CASE WHEN section_id = 4 THEN section_img END) AS client_status,
    MAX(CASE WHEN section_id = 6 THEN section_img END) AS team_status,
    MAX(CASE WHEN section_id = 7 THEN section_img END) AS bank_status
FROM `tb_section_icon`
