...
/* INITIALIZATION COLOURPICKER FOR ELEMENTS
    *********************************************/
var newColorColorPickElements = $('.Center-Block');
...
*********************************************/
var  newColorColorPickText = $('.text-color-p');
...
var newBackPickText = $('.content-main');
$('#picker-back-colour').colpick({
    layout: 'hex',
    submit: 1,
    colorScheme: 'dark',
    onChange: function(hsb, hex, rgb, el, bySetColor) {
    ...
    setNewBackColor(this.value);
    $(this).colpickSetColor(this.value);
});


function setNewBackColor(newColor) {
    newBackPickText.css('background', '#' + newColor);
  /*  if(!(/Trident\/7\./).test(navigator.userAgent)) {
        if (localStorage.getItem('textColour') != newColor) localStorage.setItem("textColour", newColor);
    }*/
}
