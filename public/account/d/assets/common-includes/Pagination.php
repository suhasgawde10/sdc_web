...
class Pagination{
	...
	var $numLinks		=  2;
	var $currentPage	=  0;
	var $firstLink   	= '&lsaquo;&lsaquo;';
	var $nextLink		= 'NEXT &gt;';
	var $prevLink		= '&lt; PREV';
	var $lastLink		= '&rsaquo;&rsaquo;';
	...
	/*	$start = (($this->currentPage - $this->numLinks) > 0) ? $this->currentPage - ($this->numLinks - 1) : 1;
		...
		if  ($this->currentPage != 1){
            $this->anchorClass = ' style="float: left;margin-left: 7px;" ';
	...
	/*	for ($loop = $start -1; $loop <= $end; $loop++){
		...
		if ($this->currentPage < $numPages){
            $this->anchorClass = ' style="float: right;margin-right: 7px;" ';
	...
	function getAJAXlink( $count, $text) {
        ...
        $this->additionalParam = "{'page' : $pageCount,'user_id' : $this->user_id}";

	    return "<a href=\"javascript:void(0);\" " . $this->anchorClass . "
				onclick=\"$.post('". get_url_param($this->baseURL)."', ". $this->additionalParam .", function(data){
